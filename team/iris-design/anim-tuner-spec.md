...
**Amendment history:**
- 2026-06-12 (`86ca88nvd`) — **Scene picker row added to the tuner (separate spec doc).** A NEW `── Scene ──` section (a native `<select>` for the per-pose tile backdrop, reusing this tuner's char/anim selectors + write-target chooser + persistence banner + live-preview + `ui:save-playback-override` round-trip) is specified in full in **`team/iris-design/scene-per-pose-spec.md`** (the scene-per-pose-per-character feature, 8 locked sponsor decisions). The scene field rides this tuner's EXISTING save message (`override.sceneId`), not a new message type; it placed `── Scene ──` between Mode and `── Write target ──`. See that doc for the picker control-type decision, the 3-layer scene cascade, the crossfade, the degrade paths, and the LOCKED vocabulary contract.
