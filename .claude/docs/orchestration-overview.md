...
- **ENTRY-number collision in `clickup-pending.md`** — parallel sub-agent dispatches each pick the next sequential `ENTRY NNN:` from current main, so two dispatches in the same round → same N → merge conflict on the second PR. Recovery: orchestrator rebases the colliding PR on current main and drops the colliding commit (`git rebase --skip` at the conflict); canonical entry for the dropped flip is added by the orchestrator post-merge as part of the merge-flip pair. Prevention applied 2026-05-24: timestamp-based IDs per `.claude/agents/dispatch-template.md` § Status-flip queue. Legacy sequential IDs in entries dated before 2026-05-24 remain as historical. Hit 1× M2-era — PR #22 (Felix M2-01) and PR #19 (Nora M2-09) both took ENTRY 014 for different tickets. **Important scope clarification (added 2026-05-26):** timestamp-based IDs prevent the *N=N number collision* failure mode above, but they do NOT prevent the *file-level additive merge conflict* class — two parallel PRs each appending to `team/log/clickup-pending.md` will still produce a merge conflict on the file when the second PR attempts to merge against the new main containing the first PR's commit. Resolution is documented in the "Orch-side rebase conflict resolution" entry below (rebase second PR + manually merge to preserve BOTH adds). **Planning consequence:** when dispatching N parallel PRs that each queue clickup-pending entries, the orchestrator should expect N-1 of them to require rebase before merge — plan the merge sequence and rebase author-side, not orch-side, when the surrounding PR is code-class (orch-side rebase is only for log-only-conflict on orch-doc PRs). Hit 1× 2026-05-26 — PR #68 (Maya Obs 6b) merged cleanly at `4669ae0`; PR #69 (Felix Obs 6a) then hit the file-level conflict; Felix author-rebased + force-pushed-with-lease to resolve.
...
- **API 529 Overloaded can hit at completion-report time AFTER substantive work — verify before retrying** — distinct from the 0-token failure pattern (`tool_uses: 0, total_tokens: ~0, status=completed, result=API Error: 529 Overloaded` — work never started, safe to retry per existing session-save lesson). 529 can ALSO hit during the agent's final-report-generation phase AFTER work completes — telemetry shows `tool_uses > 0`, non-trivial `duration_ms`, `status=completed`, `result=API Error: 529 Overloaded`. The work IS done; only the orchestrator-bound ≤200-word final report was lost. Blindly retrying creates duplicate-work race (the sub-agent re-opens the same PR or re-pushes the same branch). **Verification ritual BEFORE retrying:** (a) `gh pr list --author "@me" --state open` — did the agent open a PR? (b) `git -C <worktree> log --oneline -5 origin/main..HEAD` — did the agent push commits? (c) ClickUp ticket status via `mcp__clickup__update_task` read or `get_task_details` — did the agent flip `in progress → in review`? If ANY of (a)/(b)/(c) shows work landed → DO NOT retry. Treat as "completed without final report" — verify CI + read PR body to extract the verdict + proceed to peer-review/merge. If all empty → retry with a fresh agent name per the 0-token recipe. Hit 1× 2026-05-27 — Felix dispatch `86c9zq9vm` (host plumb Pt 1 of `86c9zmyef`) returned 529 after 115 tool uses + ~30 min runtime; PR #97 opened with commit `ffd6400`, CI green, MERGEABLE, ClickUp at `in review` — work fully landed; orchestrator verified via the 3-step ritual + proceeded to peer-review (no retry).
