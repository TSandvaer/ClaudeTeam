...
- **Squash-merge incompatible with branch-internal merge commit** — when a sub-agent's branch has a `git merge origin/main` commit resolving a conflict, GitHub's `gh pr merge --squash` still fails with `is not mergeable: the merge commit cannot be cleanly created`. Squash-merge computes the diff from the merge-base, ignoring the branch's intermediate conflict resolution. Workarounds: (a) `git rebase origin/main` then force-push (requires user permission under auto mode); (b) `git cherry-pick` the branch's commits onto a new branch from current main, close the old PR, open a new PR (no force-push needed). **DO NOT** use `git reset --soft` as a workaround — see next entry. Observed 2026-05-23 — PR #5 (Bram's M1-02) had to be re-opened as PR #6.
- **`git reset --soft` is NOT a linearization tool** — soft-reset preserves the BRANCH'S TREE STATE (all files on disk as they were at the branch tip). A branch forked from an old base, soft-reset to current `origin/main`, then committed produces a single commit whose tree = the branch's tree = MISSING every file added to main since the fork point. Committing this **deletes that newer content on merge**. Symptom: PR #8 (an attempted linearization of Felix's M1-01 via `git reset --soft origin/main && git commit`) deleted 10 files added by PR #3 + PR #6 + commit `89fa86f` (Iris's spec, Bram's 7 fixtures + research note, 3 failure-mode doc entries, dispatch-template ClickUp lifecycle update). Recovery: `git checkout <prior-sha> -- <paths>` to restore each file from its source commit, then commit + push. **The correct linearization tools are `git rebase` and `git cherry-pick`** — both apply only the branch's diffs onto the new base, never the inverse-deletion of everything-not-on-branch. Observed 2026-05-23 — destructive PR #8 recovered in commit `f870bef`.
- **Peer-reviewer worktree blocks `gh pr merge --delete-branch`.** When a sub-agent does `gh pr checkout <N>` to verify a PR's done-when test locally, their worktree gets bound to the author's branch. If they don't `git switch --detach HEAD` before posting their final report, the orchestrator's subsequent `gh pr merge --admin --squash --delete-branch` succeeds the remote merge but fails the local branch delete with `cannot delete branch ... used by worktree at <reviewer-wt>`. Recovery: detach the reviewer's worktree, `git branch -D <branch>` locally, then `git pull --ff-only origin main` to pick up the squash-merge that already happened on remote. **Prevention:** every peer-review dispatch brief must end with the detach step. Same applies to author dispatches — final step of every persona dispatch SHOULD be `git switch --detach HEAD` so worktrees never hold a branch the orchestrator needs to delete. Observed twice 2026-05-23: PR #12 (Maya reviewing Felix's M1-06) and PR #13 (Felix's worktree still on his own M1-07 branch when Maya's review-and-merge cycle hit).
